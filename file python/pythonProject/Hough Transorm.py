...
def my_hough(img, rho=1, theta=np.pi / 180, threshold=100):
    ...
    print('[My Hough] Img Height: %d | Img Width: %d | Img Diagonal Length: %d' % (
    img_height, img_width, diagonal_length))
    ...
    idx = np.squeeze(cv2.findNonZero(
        img))  # dim: 4468 x 2 (example, number of rows = number of white pixel on image processed by canny edge algorithm!)
    ...
    vote_matrix = np.dot(idx,
                         np.transpose(theta_matrix))  # => (4468 x 2) * (180 x 2)T = (4468 x 2) * (2 x 180) = 4468 x 180
    ...
    print('[My Hough] Sum of Edge Matrix = %d | Max = %d | Min = %d' % (
    int(np.sum(edge_matrix)), int(np.max(edge_matrix)), int(np.min(edge_matrix))))
...
def main():
    ...
    lines = cv2.HoughLines(edges, rho=1, theta=np.pi / 180,
                           threshold=100)
    # https://docs.opencv.org/master/dd/d1a/group__imgproc__feature.html#ga46b4e588934f6c8dfd509cc6e0e4545a
    for line in lines:
        rho, theta = line[0]
        a = np.cos(theta)
        b = np.sin(theta)
    ...
    plt.title('[OpenCV Hough] Number of lines: %d' % len(lines))
    plt.axis('off')
    ...
    plt.title('[My Hough] Number of lines: %d' % len(lines))
    plt.axis('off')
